...
function transcodeAndUpload(inputBucketName, inputFileName, outputBucketName, resolution) {
        ...
        return new Promise((resolve, reject) => {
            try {
                const inputStream = storage
                    ...
                    .file(inputFileName)
                    .createReadStream();
                const outputFileName = `${inputFileName.replace(".mp4", "")}/${inputFileName.replace(".mp4", "")}-${resolution.name}.mp4`;
                ...
                const ffmpegCommand = (0, fluent_ffmpeg_1.default)()
                    ...
                    console.log(`Processing ${resolution.name}: ${Math.round(progress.percent || 0) * 100}% done`);
                })
                    .on("end", () => {
                    console.log(`Transcoding completed for ${resolution.name} and uploaded to GCP`);
                    resolve();
                })
                    .on("error", (err) => {
    ...
    return __awaiter(this, void 0, void 0, function* () {
        try {
            ...
            subscription.on("message", (message) => __awaiter(this, void 0, void 0, function* () {
                ...
                try {
                    const [exists] = yield storage
                        .bucket(inputBucketName)
                        .file(fileName)
                        .exists();
                    if (!exists) {
                        throw new Error(`Input file gs://${inputBucketName}/${fileName} does not exist`);
                    }
                ...
                const transcodePromises = RESOLUTIONS.map((resolution) => transcodeAndUpload(inputBucketName, fileName, outputBucketName, resolution));
                // Wait for all transcoding to complete
                yield Promise.all(transcodePromises);
